   ...
   private GroupEdtorListener mListener;
    ...
    public View getChildView(final int groupPosition, final int childPosition, final boolean isLastChild, View convertView, final ViewGroup parent) {
        ...
        if (goodsInfo != null) {
            ...
            if(split.length>1){
                Picasso.with(context).load(split[0]).placeholder(R.mipmap.ic_launcher).into(cholder.iv_adapter_list_pic);
            }else{
                Picasso.with(context).load(goodsImg).placeholder(R.mipmap.ic_launcher).into(cholder.iv_adapter_list_pic);
            }
           // Picasso.with(context).load().placeholder(R.mipmap.ic_launcher).into(cholder.iv_adapter_list_pic);
