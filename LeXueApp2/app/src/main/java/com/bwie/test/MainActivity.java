...
public class MainActivity extends AppCompatActivity implements ViewPager.OnPageChangeListener{
    ...
    private void initPoint() {
        ...
        for (int i = 0;i<size;i++){
            ...
            iv_point.setPadding(30,10,30,10);//left,top,right,bottom
            ivPointArray[i] = iv_point;
            //第一个页面需要设置为选中状态，这里采用两张不同的图片
            if (i == 0){
                iv_point.setBackgroundResource(R.mipmap.dot_focus);
            }else{
    ...
    private void initViewPager() {
        ...
        //实例化图片资源
        imageIdArray = new int[]{R.mipmap.b,R.mipmap.h,R.mipmap.xz};
    ...
    public void onPageSelected(int position) {
        ...
        for (int i = 0;i<length;i++){
            ivPointArray[position].setBackgroundResource(R.mipmap.dot_focus);
            if (position != i){
